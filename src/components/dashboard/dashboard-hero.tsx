...
const columnDots = ["#94a3b8", "#818cf8", "#fbbf24", "#34d399"];
...
export function DashboardHero({ onCreateProject }: DashboardHeroProps) {
  return (
    <section className="relative mb-10 md:mb-12 overflow-hidden rounded-2xl border border-border/60 glass-card">
      ...
      />
      <div
        className="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-l from-indigo-500/[0.04] to-transparent pointer-events-none hidden md:block"
        aria-hidden
      />
      ...
      <div
        className="absolute -right-12 -bottom-12 h-56 w-56 rounded-full bg-indigo-500/10 blur-3xl pointer-events-none"
        aria-hidden
      />
      <div
        className="absolute right-8 top-6 h-32 w-32 rounded-full bg-violet-500/10 blur-2xl pointer-events-none hidden sm:block"
        aria-hidden
      />
    </section>
  );
